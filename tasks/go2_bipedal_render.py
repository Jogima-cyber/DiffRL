...
class Go2BipedalRender(Go2Bipedal):
    ...
    def post_physics_step(self):
        ...
        self.torques_hist.append(self.torques.clone().cpu().squeeze().numpy())
        ...
        if self.reset_env_buf[0] == 1.0 and len(self.video_buffer) > 0:
            ...
            plt.figure(figsize=(10, 8))  # Increased height for two subplots

            # First subplot for actions_hist
            ...
            # Second subplot for torques_hist
            plt.subplot(2, 1, 2)  # 2 rows, 1 column, position 2
            plt.plot(self.torques_hist, color='orange')
            plt.xlabel('timestep')
            plt.ylabel('Torque Value')
            plt.title('Torque Values')
            plt.legend()
            plt.grid(True)
