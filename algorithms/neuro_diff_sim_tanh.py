        ...
        with torch.no_grad():
            ...
            while filled_nb < self.min_replay:
                if self.env_type == "dflex":
                    ...
                    actions = torch.tanh(self.actor(self.env.full2partial_state(obs), deterministic = deterministic))
                    if hasattr(self.env, "action_scale_prime"):
                        actions = actions * self.env.action_scale_prime + self.env.action_bias
                    self.last_last_action_recorded = self.last_action_recorded.clone()
                    self.last_action_recorded = torch.tanh(actions.clone())
        ...
        for i in range(self.steps_num):
            ...
            if self.unroll_img:
                ...
                if self.env_type == "dflex":
                    ...
                    actions = torch.tanh(self.actor(self.env.full2partial_state(obs.clone()), deterministic = deterministic))
            if hasattr(self.env, "action_scale_prime"):
                actions = actions * self.env.action_scale_prime + self.env.action_bias
            last_last_actions[i + 2] = torch.tanh(actions.clone())
            last_actions[i + 1] = torch.tanh(actions.clone())
            self.last_last_action_recorded = self.last_action_recorded.clone()
            self.last_action_recorded = torch.tanh(actions.detach().clone())
            ...
            if self.learn_reward:
                ...
                recalculated_rew = self.env.diffRecalculateReward(real_next_obs, torch.tanh(actions), last_actions[i], last_last_actions[i], imagined_trajs = self.imagined_batch_size)
                #if (rew != recalculated_rew).any() and not self.unroll_img:
                if not torch.allclose(rew[:self.num_envs], recalculated_rew[:self.num_envs], rtol=1e-05, atol=1e-04, equal_nan=False) and not self.unroll_img: # and self.env_type == "dflex":
