...
class PMO:
    def __init__(self, cfg):
        ...
        self.update_epochs = 3 # 3
        self.minibatch_size = self.steps_num * self.num_envs // 4 # // 4
        self.clip_coef = 0.2
        self.ent_coef = 0. # 1e-5
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        #gamma_buf = torch.ones(self.steps_num, self.num_envs, dtype = torch.float32, device = self.device)
        next_values = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        action_gradients = torch.zeros((self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        rew_acc = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        #rew_acc_buf = torch.zeros((self.steps_num, self.num_envs), dtype = torch.float32, device = self.device)
        ...
        for i in range(self.steps_num):
            ...
            with torch.no_grad():
                #actions, logprobs, _ = self.actor.forward_with_log_probs(obs)
                #actions = models.dyn_model.GradientExtractorFunction.apply(actions, action_gradients[i])
                self.logprobs_buf[i] = logprobs.detach().clone()
            ...
            if self.unroll_img:
                ...
                if self.multi_modal_cor:
                    ...
                    real_next_obs, next_obs, rew, done, extra_info = models.dyn_model.DynamicsFunction.apply(raw_obs, actions, self.dyn_model, self.env, obs_rms)
            ...
            ###rew_acc[i + 1, :] = rew_acc[i, :] + rew ##

            #gamma_buf[i] = gamma.clone()
            #rew_acc_buf[i, :] = rew_acc[i + 1, :].clone()
            if i < self.steps_num - 1:
                actor_loss = actor_loss + (- rew_acc[i + 1, done_env_ids] - self.gamma * gamma[done_env_ids] * next_values[i + 1, done_env_ids]).sum()
                ###actor_loss = actor_loss + (- rew_acc[i + 1, done_env_ids] - next_values[i + 1, done_env_ids]).sum() ##
            else:
                # terminate all envs at the end of optimization iteration
                actor_loss = actor_loss + (- rew_acc[i + 1, :] - self.gamma * gamma * next_values[i + 1, :]).sum()
                ###actor_loss = actor_loss + (- rew_acc[i + 1, :] - next_values[i + 1, :]).sum() ##
        ...
        actor_loss.backward()
        #old_actor_loss = actor_loss.detach().cpu().item()
        ...
        ###### compute advantage ######
        """true_action_gradients = torch.zeros((self.steps_num, self.num_envs, self.num_actions), dtype = torch.float32, device = self.device)
        actor_loss = torch.tensor(0., dtype = torch.float32, device = self.device)
        qvalues_acc = torch.zeros((self.steps_num + 1, self.num_envs), dtype = torch.float32, device = self.device)
        qvalues_acc[-1] = self.next_values[-1].clone()
        #print("##########")
        for i in reversed(range(self.steps_num)):
            #if i < self.steps_num - 1:
            #    done_env_ids = self.dones_buf[i].nonzero(as_tuple = False).squeeze(-1)
            #    #torch.set_printoptions(precision = 10)
            #    #print(i, qvalues_acc[i + 1, done_env_ids] - rew_acc_buf[-1, done_env_ids] - self.gamma * gamma_buf[-1, done_env_ids] * next_values[-1, done_env_ids])
            #    #print("comparison", rew_acc_buf[-1, done_env_ids] + self.gamma * gamma_buf[-1, done_env_ids] * next_values[-1, done_env_ids])
            #    actor_loss = actor_loss - (self.dones_buf[i] * qvalues_acc[i + 1]).sum()

            qvalues_acc[i] = self.gamma * (1. - self.dones_buf[i]) * qvalues_acc[i + 1] + self.rew_buf[i] + self.gamma * self.dones_buf[i] * self.next_values[i]
            if i != 0:
                (-qvalues_acc[i].sum()).backward(retain_graph = True)
            else:
                (-qvalues_acc[i].sum()).backward(retain_graph = False)
            true_action_gradients[i] = action_gradients[i].clone()

        ##actor_loss = actor_loss - qvalues_acc[0].sum()
        ##self.actor_loss = actor_loss.detach().cpu().item()
        #if old_actor_loss != self.actor_loss:
        ##print(old_actor_loss - self.actor_loss)
        #    exit(0)
        #self.actor_optimizer.zero_grad()
        ##actor_loss.backward()

        action_gradients = true_action_gradients
        self.rew_buf = self.rew_buf.detach()
        qvalues_acc = qvalues_acc.detach()
        self.next_values = self.next_values.detach()"""
        ###############################
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, self.num_envs * self.steps_num, self.minibatch_size):
                ...
                diff_actions, logprobs, diff_entropy = self.actor.forward_with_log_probs(obs_rms.normalize(self.raw_obs_buf[mb_inds[:, 1], mb_inds[:, 0]]), self.debug_act_buf[mb_inds[:, 1], mb_inds[:, 0]].detach())
                logratio = (logprobs.detach() - self.logprobs_buf[mb_inds[:, 1], mb_inds[:, 0]])
                self.clipfracs += ((logratio.exp() - 1.0).abs() > self.clip_coef).float().tolist()

                to_branch_grads = action_gradients[mb_inds[:, 1], mb_inds[:, 0]] * logratio.exp().unsqueeze(-1)
                to_branch_grads /= self.minibatch_size
                dummy_reward = models.dyn_model.GradientBranchingFunction.apply(diff_actions, to_branch_grads)
                actor_loss = dummy_reward.mean() - diff_entropy.mean() * self.ent_coef
        ...
        self.clipfracs = []
        for epoch in range(self.update_epochs):
            b_inds = torch.randperm(self.num_envs)
            minibatch_size = self.num_envs // 4
            for start in range(0, self.num_envs, minibatch_size):
                end = start + minibatch_size
                mb_inds = b_inds[start:end]

                raw_obs = self.raw_obs_buf[:, mb_inds].flatten(0, 1)
                actions = self.debug_act_buf[:, mb_inds].flatten(0, 1)

                diff_actions, logprobs, diff_entropy = self.actor.forward_with_log_probs(obs_rms.normalize(raw_obs), actions)
                logratio = (logprobs.detach() - self.logprobs_buf[:, mb_inds].flatten(0, 1))
                self.clipfracs += ((logratio.exp() - 1.0).abs() > self.clip_coef).float().tolist()

                to_branch_grads = action_gradients[:, mb_inds].flatten(0, 1) * logratio.exp().unsqueeze(-1)
                to_branch_grads /= minibatch_size * self.steps_num
                dummy_reward = models.dyn_model.GradientBranchingFunction.apply(diff_actions, to_branch_grads)
                actor_loss = dummy_reward.mean() - diff_entropy.mean() * self.ent_coef
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            if self.lr_schedule == 'linear':
                #schedule_max_epoch = 500.
                actor_lr = (1e-5 - self.actor_lr) * float(epoch / self.max_epochs) + self.actor_lr
                #actor_lr = (5e-5 - self.actor_lr) * float(min(epoch, schedule_max_epoch) / schedule_max_epoch) + self.actor_lr
                #if epoch > schedule_max_epoch:
                #    actor_lr = (1e-5 - 5e-5) * float((epoch - schedule_max_epoch) / (self.max_epochs - schedule_max_epoch)) + 5e-5
                for param_group in self.actor_optimizer.param_groups:
                    param_group['lr'] = actor_lr
                lr = actor_lr
                critic_lr = (1e-5 - self.critic_lr) * float(epoch / self.max_epochs) + self.critic_lr
                #critic_lr = (5e-5 - self.critic_lr) * float(min(epoch, schedule_max_epoch) / schedule_max_epoch) + self.critic_lr
                #if epoch > schedule_max_epoch:
                #    critic_lr = (1e-5 - 5e-5) * float((epoch - schedule_max_epoch) / (self.max_epochs - schedule_max_epoch)) + 5e-5
                for param_group in self.critic_optimizer.param_groups:
                    param_group['lr'] = critic_lr
                #dyn_model_lr = (1e-5 - self.dyn_model_lr) * float(epoch / self.max_epochs) + self.dyn_model_lr
                #for param_group in self.dyn_model_optimizer.param_groups:
                #    param_group['lr'] = dyn_model_lr
            else:
                lr = self.actor_lr
