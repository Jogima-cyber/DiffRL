...
class CameraMosaicWrapper(gym.Wrapper):
    """
    A Gymnasium environment wrapper that renders all available cameras
    in a MuJoCo environment as a mosaic (grid) of views in a single image.
    
    The wrapper assumes:
      • The wrapped env has a MuJoCo model accessible as `env.model`,
      • The number of cameras is given by `env.model.ncam`,
      • The env’s render method (when passed mode="rgb_array") uses the attribute
        `env.camera_id` (or equivalently, the underlying renderer’s `camera_id`)
        to choose the camera view.
    
    When you call render("rgb_array") (or "human"), the wrapper iterates over the
    specified (or all) camera IDs, gathers each rendered image, and stitches
    them into one mosaic image.
    """
    def __init__(self, env, mosaic_rows=None, mosaic_cols=None, camera_ids=None):
        super().__init__(env)
        # If no camera_ids are provided, use all cameras from the model.
        if camera_ids is None:
            if hasattr(self.env, "model") and hasattr(self.env.model, "ncam"):
                self.camera_ids = list(range(self.env.model.ncam)) + [-1]
            else:
                raise AttributeError(
                    "The wrapped environment does not expose a model with 'ncam'."
                )
        else:
            self.camera_ids = camera_ids

        self.num_cams = len(self.camera_ids)
        # Determine mosaic grid layout automatically if not provided.
        if mosaic_rows is None or mosaic_cols is None:
            mosaic_rows = int(np.ceil(np.sqrt(self.num_cams)))
            mosaic_cols = int(np.ceil(self.num_cams / mosaic_rows))
        self.mosaic_rows = mosaic_rows
        self.mosaic_cols = mosaic_cols

    def render(self, mode="rgb_array"):
        views = []
        # Backup the original camera id if it exists.
        orig_camera_id = getattr(self.env, "camera_id", None)
        # Render each camera view.
        for cam_id in self.camera_ids:
            self.env.mujoco_renderer.camera_id = cam_id
            # Render an image from the current camera.
            img = self.env.render()
            if cam_id > 0 and cam_id <= 3:
                img = np.rot90(img, k = 2)

            views.append(img)

        # Restore the original camera id.
        if orig_camera_id is not None:
            self.env.camera_id = orig_camera_id

        # Make sure there is one image per grid cell.
        if len(views) < (self.mosaic_rows * self.mosaic_cols):
            h, w, c = views[0].shape
            n_missing = (self.mosaic_rows * self.mosaic_cols) - len(views)
            blank = np.zeros((h, w, c), dtype=views[0].dtype)
            views += [blank] * n_missing

        # Assume all images are identical in size.
        h, w, c = views[0].shape
        # Build mosaic grid: arrange images row by row.
        mosaic_rows = []
        for i in range(self.mosaic_rows):
            row_imgs = views[i * self.mosaic_cols : (i + 1) * self.mosaic_cols]
            row = np.concatenate(row_imgs, axis=1)  # horizontal stacking
            mosaic_rows.append(row)
        mosaic = np.concatenate(mosaic_rows, axis=0)  # vertical stacking

        if mode == "rgb_array":
            return mosaic
        elif mode == "human":
            cv2.imshow("Camera Mosaic", cv2.cvtColor(mosaic, cv2.COLOR_RGB2BGR))
            cv2.waitKey(1)
            return mosaic
        else:
...
class DeMO_SAPO:
    def __init__(self, cfg, env = None):
        ...
        if self.env_type == "dflex":
            ...
            self.env = env
            self.render_env = metaworld.envs.ALL_V2_ENVIRONMENTS_GOAL_OBSERVABLE[cfg["params"]["metaworld"]["name"]](seed=cfg["params"]["general"]["seed"], render_mode="rgb_array")

            self.render_env.model.opt.enableflags |= mujoco.mjtEnableBit.mjENBL_OVERRIDE
            self.render_env.model.opt.o_solimp[0] = 0.
            self.render_env.model.opt.o_solimp[2] = 0.15
            self.render_env.model.opt.o_margin = 0.15

            self.render_env = CameraMosaicWrapper(self.render_env)
            trigger = lambda t: t % 1 == 0
            self.render_env = RecordVideo(self.render_env, video_folder=os.path.join(self.log_dir, 'save_videos'), episode_trigger=trigger, disable_logger=False)
            self.render_obs, _ = self.render_env.reset()

            self.env.num_actions = self.env.action_space.shape[-1]
            self.max_episode_length = self.render_env.env.env.max_path_length
        ...
        self.clipped_double_critic = cfg["params"]["config"].get("clipped_double_critic", False)
        CRITIC_NB = 10
        if self.env_type == "dflex":
            if self.clipped_double_critic:
                self.critic = [critic_fn(self.num_obs, cfg['params']['network'], device = self.device) for _ in range(CRITIC_NB)]
            else:
            ...
            if self.asymetrical_value:
                if self.clipped_double_critic:
                    self.critic = [critic_fn(self.num_dyn_obs, cfg['params']['network'], device = self.device) for _ in range(CRITIC_NB)]
                else:
                    self.critic = critic_fn(self.num_dyn_obs, cfg['params']['network'], device = self.device)
            else:
                if self.clipped_double_critic:
                    self.critic = [critic_fn(self.num_act_obs, cfg['params']['network'], device = self.device) for _ in range(CRITIC_NB)]
                else:
        ...
        self.smooth_gradient = cfg["params"]["config"].get("smooth_gradient", False)
        ...
        self.dyn_loss_grad = 0.

        if self.smooth_gradient:
            self.dyn_model_copy = dyn_model_fn(self.num_dyn_obs, self.num_actions, cfg['params']['network'], self.learn_reward, device = self.device)
            self.dyn_model_copy.load_state_dict(self.dyn_model.state_dict())
        ...
        if self.clipped_double_critic:
            def target_critic(obs):
                ...
                targets_selected = torch.randperm(len(self.critic))[:2]
                target_preds = preds[..., targets_selected]
                return preds.mean(-1), target_preds.min(-1).values
        ...
        if self.avantage_objective:
            ...
            if self.clipped_double_critic:
                #self.critic_optimizer = torch.optim.Adam(itertools.chain(*([c.parameters() for c in self.critic])), betas = cfg['params']['config']['betas'], lr = self.critic_lr)
                ...
                #self.critic_optimizer = torch.optim.AdamW(itertools.chain(*([c.parameters() for c in self.critic])), lr = self.critic_lr)
            else:
        ...
        self.ent_buf = torch.zeros((self.steps_num, value_num_envs + self.imagined_batch_size), dtype = torch.float32, device = self.device)
        ...
        self.target_entropy = -12. # -self.num_actions / 2 #/!\#/!\#
        #self.target_entropy = -self.num_actions / 2 #/!\#/!\#
        self.init_target_entropy = self.target_entropy
        self.target_ent_decay = cfg["params"]["config"].get("target_ent_decay", False)
            ...
            while filled_nb < self.min_replay:
                if self.env_type == "dflex":
                    if self.no_stoch_act_model:
                        actions, _, self.act_hidden_in = self.actor(obs, deterministic = True, l = self.act_hidden_in)
                    else:
                    ...
                    if self.no_stoch_act_model:
                        actions, _, self.act_hidden_in = self.actor(self.env.full2partial_state(obs), deterministic = True, l = self.act_hidden_in)
                    else:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        next_values_target = torch.zeros((self.steps_num + 1, self.num_envs + self.imagined_batch_size), dtype = torch.float32, device = self.device)
        ...
        if self.env_type == "dflex":
            ...
            obs = torch.tensor(self.env.observations, device = self.device, dtype = torch.float32)
        else:
            obs = self.env.dyn_obs_buf.clone()
        ...
        for i in range(horizon):
            ...
            if self.env_type == "dflex" or self.env_type == "metaworld":
                if self.no_stoch_act_model:
                    ...
                    actions, ent, self.act_hidden_in = self.actor(obs, deterministic = deterministic, l = self.act_hidden_in)
                if self.log_gradients:
                    actions_, ent_, _ = self.actor(obs_, deterministic = deterministic, l = None)
                    actions_diff, ent_diff, _ = self.actor(obs_diff, deterministic = deterministic, l = None)
            else:
                if self.no_stoch_act_model:
                    actions, _, self.act_hidden_in = self.actor(self.env.full2partial_state(obs.clone()), deterministic = True, l = self.act_hidden_in)
                else:
                    actions, ent, self.act_hidden_in = self.actor(self.env.full2partial_state(obs.clone()), deterministic = deterministic, l = self.act_hidden_in)
            ...
            if self.unroll_img:
                ...
                if self.multi_modal_cor:
                    ...
                    if self.dyn_recurrent:
                        ...
                        if self.vae:
                            img_next_obs_delta, _, self.p_hidden_in = self.dyn_model(obs.unsqueeze(-2), actions.unsqueeze(-2), obs_rms.normalize(real_next_obs.detach()).unsqueeze(-2), self.p_hidden_in)
                        else:
                            if self.act_rms is not None:
                                ...
                                if self.smooth_gradient:
                                    img_next_obs_delta, _, self.p_hidden_in = self.dyn_model_copy(obs.unsqueeze(-2), actions.unsqueeze(-2), self.p_hidden_in)
                                else:
                                    img_next_obs_delta, _, self.p_hidden_in = self.dyn_model(obs.unsqueeze(-2), actions.unsqueeze(-2), self.p_hidden_in)
            ...
            if self.filter_sigma_events and (self.env_type == "dflex" or self.env_type == "metaworld"):
            ...
            if self.learn_reward:
                if self.obs_rms is not None:
                    if self.smooth_gradient:
                        recalculated_rew = self.dyn_model_copy.reward(obs_rms.normalize(real_next_obs), actions).squeeze(-1)
                    else:
                        recalculated_rew = self.dyn_model.reward(obs_rms.normalize(real_next_obs), actions).squeeze(-1)
                else:
                    if self.smooth_gradient:
                        recalculated_rew = self.dyn_model_copy.reward(real_next_obs, actions).squeeze(-1)
                    else:
                        recalculated_rew = self.dyn_model.reward(real_next_obs, actions).squeeze(-1)
                # models.dyn_model.RewardsFunction.apply(raw_obs, raw_actions, rew.clone(), self.dyn_model, obs_rms)
            else:
            ...
            if self.filter_sigma_events or self.env_type == "metaworld":
                def create_hook(debug = False, name = None):
                    def hook(grad):
                        if debug:
                            if torch.isnan(grad).any():
                                print(f"grad of {name} has nan")
                            if torch.isinf(grad).any():
                                print(f"grad of {name} has inf")
                        torch.nan_to_num(grad, 0.0, 0.0, 0.0, out = grad)
                    return hook

                if real_next_obs.requires_grad:
                    real_next_obs.register_hook(create_hook(name = "real_next_obs"))
                if actions.requires_grad:
                    actions.register_hook(create_hook(name = "actions"))
                if rew.requires_grad:
                    rew.register_hook(create_hook(name = "rew"))
            ...
            if self.smooth_gradient:
                second_diff_actions = raw_actions.detach().clone()
                ...
                if self.learn_reward:
                    if self.obs_rms is not None:
                        second_diff_rew = self.dyn_model.reward(obs_rms.normalize(second_diff_real_next_obs), torch.tanh(second_diff_actions)).squeeze(-1)
                    else:
                        second_diff_rew = self.dyn_model.reward(second_diff_real_next_obs, torch.tanh(second_diff_actions)).squeeze(-1)
                else:
                    second_diff_rew = self.env.diffRecalculateReward(second_diff_real_next_obs, torch.tanh(second_diff_actions), last_actions[i], last_last_actions[i], imagined_trajs = self.imagined_batch_size)
            ...
            if self.avantage_objective:
                actor_loss = actor_loss - self.critic_adv(prev_obs, actions).sum()
            ...
            with torch.no_grad():
                if self.decouple_value_actors:
                    ...
                    #self.rew_buf[i] = rew_wt_ent[:self.apg_num_actors].clone() #/!\#
                    if i < horizon - 1:
                        self.done_mask[i] = done[:self.apg_num_actors].clone().to(torch.float32)
                    else:
                        self.done_mask[i, :] = 1.
                    if self.clipped_double_critic:
                        self.next_values[i] = next_values_target[i + 1, :self.apg_num_actors].clone()
                    else:
                    ...
                    #self.rew_buf[i] = rew_wt_ent.clone() #/!\#
                    if i < horizon - 1:
        ...
        if self.smooth_gradient:
            dyn_loss /= horizon * self.num_envs ###
        ...
        if self.smooth_gradient:
           self.dyn_loss_grad = dyn_loss.detach().cpu().item() ###
    ...
    def train_dyn_model(self):
        ...
        for ts in range(train_steps):
            ...
            if self.obs_rms is not None:
                ...
                #with torch.no_grad():
                #    self.obs_rms.update(observations_concat.flatten(0, 1))

                if self.vae or self.learn_reward:
                    next_state = self.obs_rms.normalize(next_observations_concat.clone())
            else:
                state = observations_concat
                if self.vae or self.learn_reward:
                    next_state = next_observations_concat
            #######################################################
            #NO_DYN_NORM#state = observations_concat.clone() # /!\ to remove TMP #NO_DYN_NORM#
            #######################################################
            ...
            if self.vae:
                if self.dyn_recurrent:
                    ...
                    pred = self.dyn_model.encode_decode(state, actions_concat, next_state)
                ...
                pred_next_obs = pred[0]
                if self.learn_reward:
                    pred_reward = self.dyn_model.raw_reward(next_state, actions_concat)
    ...
    def train(self):
        ...
        for epoch in range(self.max_epochs):
            ...
            if self.target_ent_decay:
                self.target_entropy = (0. - self.init_target_entropy) * float(epoch / self.max_epochs) + self.init_target_entropy
            ...
            if self.smooth_gradient:
                self.writer.add_scalar('dyn_loss/step', self.dyn_loss_grad, self.step_count) ###
                self.writer.add_scalar('dyn_loss/iter', self.dyn_loss_grad, self.iter_count) ###
            ...
            self.writer.add_scalar('entropy/target_ent/step', self.target_entropy, self.step_count)
            self.writer.add_scalar('entropy/target_ent/iter', self.target_entropy, self.iter_count)
            ...
            if len(self.episode_loss_his) > 0:
                ...
                if mean_policy_loss < self.best_policy_loss:
                    ...
                    if self.env_type == "metaworld":
                        ...
                        while not done:
                            with torch.no_grad():
                                torch_obs = torch.tensor(self.render_obs, device = self.device, dtype = torch.float32)
                                if self.obs_rms is not None:
                                    actions, _, _ = self.actor(self.obs_rms.normalize(torch_obs), deterministic = False, l = None)
                                else:
                                    actions, _, _ = self.actor(torch_obs, deterministic = False, l = None)
                                actions = torch.tanh(actions)
                                #actions[..., -2] = -1.
                                self.render_obs, _, done, truncated, infos = self.render_env.step(actions.squeeze().cpu().numpy())
                                done = done | truncated
                        self.render_obs, _ = self.render_env.reset()
            ...
            if not self.clipped_double_critic:
                with torch.no_grad():
