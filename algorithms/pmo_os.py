...
class PMO:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        next_obs_gradients = torch.zeros((self.steps_num, self.num_envs, self.num_obs), dtype = torch.float32, device = self.device)
        ...
        for i in range(self.steps_num):
            ...
            #real_next_obs = models.dyn_model.GradientExtractorFunction.apply(real_next_obs.clone(), next_obs_gradients[i])
            ...
            real_next_obs = models.dyn_model.GradientExtractorFunction.apply(real_next_obs.clone(), next_obs_gradients[i]) # After diff reward computation
        ...
        for epoch in range(self.update_epochs):
            ...
            for start in range(0, self.num_envs * self.steps_num, self.minibatch_size):
                ...
                diff_actions, logprobs, diff_entropy = self.actor.forward_with_log_probs(obs_rms.normalize(self.raw_obs_buf[mb_inds[:, 1], mb_inds[:, 0]]), self.debug_act_buf[mb_inds[:, 1], mb_inds[:, 0]])
                logratio = (logprobs.detach() - self.logprobs_buf[mb_inds[:, 1], mb_inds[:, 0]])

                dummy_next_obs = models.dyn_model.DynamicsFunctionPMO.apply(
                    self.raw_obs_buf[mb_inds[:, 1], mb_inds[:, 0]], diff_actions, self.true_raw_next_obs_buf[mb_inds[:, 1], mb_inds[:, 0]], self.dyn_model, obs_rms
                )
                #real_next_obs = self.dyn_model(obs_rms.normalize(self.raw_obs_buf[mb_inds[:, 1], mb_inds[:, 0]]), torch.tanh(diff_actions))[0] + self.raw_obs_buf[mb_inds[:, 1], mb_inds[:, 0]]
                #to_branch_grads = action_gradients[mb_inds[:, 1], mb_inds[:, 0]] * logratio.exp().unsqueeze(-1)
                to_branch_grads = next_obs_gradients[mb_inds[:, 1], mb_inds[:, 0]] * logratio.exp().unsqueeze(-1)
                #to_branch_grads = next_obs_gradients[mb_inds[:, 1], mb_inds[:, 0]]
                dummy_reward = models.dyn_model.GradientBranchingFunction.apply(dummy_next_obs, to_branch_grads)
                #actor_loss = dummy_reward.mean() - diff_entropy.mean() * self.ent_coef
                #dummy_reward = models.dyn_model.GradientBranchingFunction.apply(real_next_obs, to_branch_grads)
                #reward = self.env.diffRecalculateReward(real_next_obs, torch.tanh(diff_actions))
                reward = self.env.diffRecalculateReward(dummy_next_obs, torch.tanh(diff_actions))
                actor_loss = reward.mean() + dummy_reward.mean() - diff_entropy.mean() * self.ent_coef
