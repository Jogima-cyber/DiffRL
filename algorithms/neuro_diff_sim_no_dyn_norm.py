...
class NeuroDiffSim:
    ...
    def compute_actor_loss(self, deterministic = False):
        ...
        for i in range(self.steps_num):
                    ...
                    if self.dyn_recurrent:
                        ...
                        if self.vae:
                            ...
                            if self.act_rms is not None:
                                ...
                                #img_next_obs_delta, _, self.p_hidden_in = self.dyn_model(obs.unsqueeze(-2), actions.unsqueeze(-2), self.p_hidden_in)
                                img_next_obs_delta, _, self.p_hidden_in = self.dyn_model(raw_obs.unsqueeze(-2), actions.unsqueeze(-2), self.p_hidden_in) #NO_DYN_NORM
            ...
            if self.learn_reward:
                ...
                if not torch.allclose(rew[:self.num_envs], recalculated_rew[:self.num_envs], rtol=1e-05, atol=1e-03, equal_nan=False) and not self.unroll_img: # and self.env_type == "dflex":
            ...
            if self.env_type == "dflex":
                ...
                if self.dyn_recurrent:
                    ...
                    next_values[i + 1] = self.target_critic(obs.clone()).squeeze(-1)
                ...
                if self.env_type == "dflex" and id < self.num_envs and (torch.isnan(extra_info['obs_before_reset'][id]).sum() > 0 \
                    ...
                    if self.env_type == "dflex":
                        ...
                        if self.dyn_recurrent:
                            ...
                            next_values[i + 1, id] = self.target_criti(real_obs.clone()).squeeze(-1)
    ...
    def compute_critic_loss(self, batch_sample):
        if self.env_type == "dflex":
            ...
            predicted_values = self.critic(batch_sample['obs']).squeeze(-1)
            #predicted_values = self.critic(self.env.full2partial_state(batch_sample['obs'])).squeeze(-1)
    ...
    def train_dyn_model(self):
        ...
        for ts in range(train_steps):
            ...
            #######################################################
            state = observations_concat.clone() # /!\ to remove TMP #NO_DYN_NORM#
            #######################################################
