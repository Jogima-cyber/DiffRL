...
class CriticAdvMLP(nn.Module):
    def __init__(self, obs_dim, action_dim, cfg_network, device='cuda:0'):
        super(CriticAdvMLP, self).__init__()

        self.device = device

        self.layer_dims = [obs_dim + action_dim] + cfg_network['critic_adv_mlp']['units'] + [1]
        ...
        for i in range(len(self.layer_dims) - 1):
            modules.append(init_(nn.Linear(self.layer_dims[i], self.layer_dims[i + 1])))
            if i < len(self.layer_dims) - 2:
                modules.append(model_utils.get_activation_func(cfg_network['critic_adv_mlp']['activation']))
                modules.append(torch.nn.LayerNorm(self.layer_dims[i + 1]))
